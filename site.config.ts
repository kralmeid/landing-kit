/**
 * Site-wide structural configuration.
 *
 * Edit this file to change brand, logos, contact wiring, navigation,
 * and which sections are rendered. Translatable copy lives in
 * messages/{pt,en}.json, not here.
 */
export const siteConfig = {
  brand: {
    name: 'Your Brand',
    domain: 'yourbrand.com',
    logo: {
      dark: '/placeholders/logo-wordmark-dark.svg',
      light: '/placeholders/logo-wordmark-light.svg',
      mark: '/placeholders/logo-mark.svg',
    },
  },
  i18n: { locales: ['pt', 'en'] as const, default: 'pt' as const },
  contact: {
    email: 'hello@yourbrand.com',
    resend: {
      fromEmail: 'noreply@yourbrand.com',
      enabled: Boolean(process.env.RESEND_API_KEY),
    },
  },
  nav: [
    { key: 'services', href: '#services' },
    { key: 'about', href: '#about' },
    { key: 'portfolio', href: '#portfolio' },
    { key: 'contact', href: '#contact' },
  ] as const,
  sections: {
    portfolio: {
      enabled: true,
      layout: 'grid-3' as const,
      images: Array.from({ length: 6 }, (_, i) => ({
        src: `/placeholders/portfolio-0${i + 1}.svg`,
        alt: `Project ${i + 1}`,
      })),
    },
    trust: { enabled: true },
  },
  social: { instagram: '', linkedin: '', github: '' },
  seo: { ogImage: '/placeholders/og-image.svg' },
  hero: {
    image: '/placeholders/hero.svg',
  },
  about: {
    portrait: '/placeholders/portfolio-01.svg',
  },
} as const;

export type SiteConfig = typeof siteConfig;
